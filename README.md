...
## What I learned
I tested my fault-handling logic by removing my BM280 sensor mid-run, but the Task Watchdog Timer fired before my fault logic could finish executing. I realized that a blocking I2C call inside the main loop was starving the watchdog timer before the fault-handling logic ever got a chance to run. I resolved this by increasing the watchdog timeout from 5 seconds to 30 seconds, and by tightening the state system during a fault 
from: FAULT -> IDLE -> FAULT -> IDLE -> FAULT -> IDLE -> SAFE_MODE 
to: FAULT -> FAULT -> FAULT -> SAFE_MODE 
